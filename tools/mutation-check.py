...
SETTINGS_MUTATIONS = [
    ...
    ("menu.js", "return x === y ? 0 : (x < y ? -1 : 1);", "return 0;",
     "действия внутри устройства не сортируются"),
    ("menu.js", "if (items.length === 1) {", "if (false) {",
     "лишний экран из одной кнопки не пропускается"),
    ("menu.js", "return item.room + ' · ' + item.device + ' · ' + item.title;",
     "return item.room + ' · ' + item.title;",
     "в подтверждении не называется устройство"),
    ("menu.js", "if (item.title === item.device) {", "if (false) {",
     "имя дублируется, когда устройство и действие названы одинаково"),
    # Подпись обязана быть геттером: иначе значение вмерзает в момент сборки меню.
    ("menu.js", "return labelWithValue(item, name);", "return name;",
     "подпись кнопки не показывает текущее значение"),
    ("menu.js", "count[items[i].title] > 1", "false",
     "одноимённые действия не различаются"),
