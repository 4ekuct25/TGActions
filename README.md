...
**Что заполнить руками** (искать «Заполнить»):

* `bots.home.key` в `index.js` — токен бота из @BotFather;
* `access.profiles[].chatId` — id семейного чата (отрицательный) и личных сообщений;
* `access.users` — `user_id` тех, кому можно. Свой id покажет команда `/who`.
