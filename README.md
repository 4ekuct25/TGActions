...
* **BOT\_DESCRIPTION / BOT\_SHORT\_DESCRIPTION** отдаются наружу как
  `botDescription` / `botShortDescription` и читаются движком оттуда. Если
  описание не задано ни у бота, ни здесь — движок пропускает вызов
  `setMyDescription` (пустая строка в Telegram стёрла бы описание) и пишет
  предупреждение в лог. Лимиты Telegram: описание ≤ 512 символов,
  короткое ≤ 120.
...
Сценарий стенда заканчивается блоком с `null`-аргументами (`bots._default` /
